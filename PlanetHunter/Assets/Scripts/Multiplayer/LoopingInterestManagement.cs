...
public class LoopingInterestManagement : InterestManagement
{
    public float worldWidth = 100f;
    public float worldHeight = 100f;
    [Tooltip("How close a player needs to be (including wrap-around) to see an object")]
    ...
    public override bool OnCheckObserver(NetworkIdentity identity, NetworkConnectionToClient newObserver)
    {
        ...
        Vector3 observerPos = newObserver.identity.transform.position;
        Vector3 targetPos = identity.transform.position;

        return IsWithinLoopingRange(observerPos, targetPos);
    }
    ...
    private bool IsWithinLoopingRange(Vector3 a, Vector3 b) {
        float dx = Mathf.Abs(a.x - b.x);
        float dy = Mathf.Abs(a.y - b.y);

        // if the flat distance is greater than half the map, distance through loop is lower
        if (dx > worldWidth / 2f) dx = worldWidth - dx;
        if (dy > worldHeight / 2f) dy = worldHeight - dy;

        // get c squared and compare
        float distanceSquared = (dx * dx) + (dy * dy);
        return distanceSquared <= (visRange * visRange);
    }
}
